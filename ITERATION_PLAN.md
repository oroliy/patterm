...
- [x] 落地会话恢复 MVP，保存激活标签、过滤器状态和基础 UI 偏好
- [x] 扩展搜索结果导航与高亮定位，补齐从搜索到原始终端位置的回跳体验
- [x] 扩展到跨标签页全局搜索，支持结果列表跳转到目标标签与具体终端条目
...
- P1: 已完成会话恢复 MVP，仅恢复 UI 与配置，不自动重连串口
- P1: 已完成搜索结果定位与高亮导航，支持结果计数、上一条/下一条跳转和当前匹配高亮
- P1: 已完成跨标签页全局搜索 MVP，支持 `All/RX/TX/Error` 范围筛选和结果跳转
...
- 已完成：会话恢复 MVP，当前可恢复断开标签、激活标签、终端过滤器和主题等 UI 状态
- 已完成：跨标签页全局搜索 MVP，可通过结果列表切换标签并定位到具体终端条目
- 已完成：第五阶段 P0 第一版，只读 Trigger 高亮已支持每标签页规则、会话恢复和命中标记
- 已完成：第五阶段 P0 第二版，Workflow MVP 已支持每标签页定义、运行/停止、等待匹配和超时失败
- 已完成：第五阶段 P1 第一版，事务块面板 MVP 已支持按时间窗口归并请求/响应块、被动接收块和回跳定位
- 已完成：第五阶段 P1 第二版，事务块面板已支持块级复制、导出和标星
- 已完成：第五阶段 P1 第三版，事务块面板已支持块级重命名，以及 `All / Failed / Starred` 筛选
- 已完成：第五阶段 P1 第四版，事务块面板已支持失败原因摘要和当前过滤结果批量导出
- 下一步：继续第五阶段，推进分屏布局 MVP
...
### 第五阶段拆解建议
- P0: 已完成 Trigger 引擎只读高亮基础版，支持 per-tab 规则、Contains/Regex 匹配、会话恢复
- P0: 已完成 Workflow 基础版，当前支持“发送步骤 + 等待匹配 + 超时失败”，暂不做脚本执行
- P1: 已完成事务块按时间窗口归并基础版，当前支持请求/响应与被动接收块、Blocks 面板和回跳定位
- P1: 已完成事务块块级动作第二版，当前支持标星、复制单块和导出单块
- P1: 已完成事务块块级动作第三版，当前支持重命名，以及 `All / Failed / Starred` 筛选
- P1: 已完成事务块块级动作第四版，当前支持失败原因摘要和可见块批量导出
