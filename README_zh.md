...
- 每个标签页支持事务块面板，可按时间窗口将 `TX -> RX` 归并为请求响应块或被动接收块，并支持回跳、复制、导出、标星、重命名，以及失败/已标星筛选
- 页头主题控件支持显式选择 `跟随系统`、`深色`、`浅色`
...
- 支持事务块面板 MVP，按时间窗口归并终端流量，并支持跳转、复制、导出、标星、重命名，以及 `全部 / 失败 / 已标星` 筛选

### 工作区控制
- 页头主题菜单支持 `跟随系统`、`深色`、`浅色`
- 重写了 About 对话框，展示当前运行端、主题、标签数量和当前能力摘要
...
9. **打开 Blocks 面板**：查看归并后的事务块，并对单个事务块执行重命名、标星、失败筛选、跳转、复制或导出
